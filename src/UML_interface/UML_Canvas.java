...
public class UML_Canvas extends JPanel{
	//default //
	Mouse_Event curr_mouse_eventEvent = null;
	MouseAdapter canvasmouseadapter;
	...
	private void Override_Mouse_Adapter() {
		...
		canvasmouseadapter = (new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (curr_mouse_eventEvent != null)
					curr_mouse_eventEvent.Mouse_Clicked(thisCanvas, e.getX(), e.getY());
			}
			//release
			public void mouseReleased(MouseEvent e) {
				if (curr_mouse_eventEvent != null)
					curr_mouse_eventEvent.Mouse_Released(thisCanvas, e.getX(), e.getY());
			}
			//Dragged
			public void mouseDragged(MouseEvent e) {
				if (curr_mouse_eventEvent != null)
                   curr_mouse_eventEvent.Mouse_Dragged(thisCanvas, e.getX(), e.getY());
			}
			//Pressed
			public void mousePressed(MouseEvent e) {
				if (curr_mouse_eventEvent != null)
                   curr_mouse_eventEvent.Mouse_Pressed(thisCanvas, e.getX(), e.getY());
			}
		});
	...
	// public //
	public void set_curr_mouse_eventEvent(Mouse_Event mouseevent) {
		curr_mouse_eventEvent = mouseevent;
	}
	...
	public void setgroup() {
		if(selectobjects.size() > 1) {
			Group_Object grouptmp = new Group_Object(selectobjects);
			addgroup(grouptmp);
			addselectObject(grouptmp);
		}
		repaint();
	}
	public void ungroup() {
